...
//#define USE_MMD4MECANIM_AGENT // Legacy.
#endif
...
public class MMD4MecanimBulletPhysics : MonoBehaviour
{
	...
	public class MMDModel
	{
		...
		Matrix4x4								_modelLocalToWorld = Matrix4x4.identity;
		Matrix4x4								_modelLocalToWorld2 = Matrix4x4.identity;
		Quaternion								_modelRotation = Quaternion.identity;
		Quaternion								_modelRotation2 = Quaternion.identity;
		Vector3									_rootPosition = Vector3.zero;
		Vector3									_rootPosition2 = Vector3.zero;
		Quaternion								_rootRotation = Quaternion.identity;
		Quaternion								_rootRotation2 = Quaternion.identity;
		bool									_savedTransform = false;
		bool									_savedTransform2 = false;
		...
		private bool _Prepare( MMD4MecanimModel model )
		{
			...
			MMD4MecanimBone rootBone = this.model.GetRootBone();
			if( rootBone != null ) {
				_rootTransform = rootBone.transform;
			}
		...
		public bool Create( MMD4MecanimModel model )
		{
			...
			if( mmdModelPtr != IntPtr.Zero ) {
				...
				_multiThreading = joinWorld.worldProperty.multiThreading;
		...
		public void Update()
		{
			...
			_modelLocalToWorld2 = _modelLocalToWorld;
			_modelRotation2 = _modelRotation;
			if( _rootTransform != null ) {
				_rootPosition2 = _rootPosition;
				_rootRotation2 = _rootRotation;
			}
			_savedTransform2 = _savedTransform;
			_modelLocalToWorld = modelTransform;
			_modelRotation = _modelTransform.rotation;
			if( _rootTransform != null ) {
				_rootPosition = _rootTransform.position;
				_rootRotation = _rootTransform.rotation;
			}
		...
		public void LateUpdate( float deltaTime )
		{
			...
			unchecked {
				if( (lateUpdateFlags & (int)LateUpdateFlags.Bone) != 0 &&
					...
					if( _multiThreading && _savedTransform2 ) { // Multithreading & 2nd LateUpdate only
					...
					for( int i = 0; i != boneListLength; ++i ) {
						Transform boneTransform = _boneTransformList[i];
						if( boneTransform != null ) {
							bool updatePosition = false; // for rootTransform
							bool updateRotation = false; // for rootTransform
							...
							if( (lateUpdateBoneFlagsList[i] & (int)LateUpdateBoneFlags.LateUpdated) != 0 ) {
								if( (lateUpdateBoneFlagsList[i] & (int)LateUpdateBoneFlags.Position) != 0 ) {
									if( boneTransform != rootTransform ) {
							...
							if( boneTransform == rootTransform ) {
								Vector3 position = _rootPosition2;
								Quaternion rotation = _rootRotation2;
								if( updatePosition ) {
									Vector3 localPositon = lateUpdateBonePositionList[i];
									position = _modelLocalToWorld2.MultiplyPoint( localPositon );
								}
								if( updateRotation ) {
									Quaternion localRotation = lateUpdateBoneRotationList[i];
									rotation = _modelRotation2 * localRotation;
								}
